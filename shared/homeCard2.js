...
export default class HomeCard2 extends React.Component {
  constructor() {
    super();
    this.state = {
      product: [],
  ...
  handleInit = async () => {
    if (auth().currentUser) {
      ...
      this.props.item.reports.map((r) => {
        if (r.email === auth().currentUser.email) {
          reportExists = true;
        }
      });
      if (reportExists === false && this.props.item.reports.length < 10) {
        var data2 = {
  ...
  sendLikedPushNotification = async (e, f) => {
    ...
    const message = {
      ...
      notification: {
        ...
        body: this.state.currentUser.name + ' liked your request',
        vibrate: 1,
        sound: 1,
