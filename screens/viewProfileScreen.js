...
sendFollowPushNotification = async (e, f) => {
    ...
    const message = {
        registration_ids: [
            this.state.currentUser.pushToken,
        ],
        notification: {
...
handleChat = async () => {
  ...
  render() {
    return (
        <SafeAreaView style={{ width: '100%', flex: 1, backgroundColor: colors.primary }}>
        <View style={styles.head}>
            ...
            <TouchableOpacity
                style={styles.action}
                onPress={() => this.handleShare(this.props.route.params.screen ? this.props.route.params.screen : this.props.route.params.id)}>
                <Ionicons
            ...
            <ScrollView style={{ width: '100%', flex: 1 }}>
                <View style={{ width: '100%', alignItems: 'center', flex: 1 }}>
                    <View style={styles.profileBox}>
                        <View style={{ width: '100%', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                            ...
                            this.state.ratings && this.state.ratings.length > 0
                                ?
                                <View style={{width:width*0.6,height:80,alignItems:'center'}}>
                                    <Swiper paginationStyle={{bottom:0}} activeDotColor={'#d65a31'} showsButtons={false}>
                                        {
                                            this.state.ratings.map(item=>{
                                                return(
                                                <View style={styles.rating}>
                                                    <Text style={styles.ratingNumber}>{item.rate}.0 <Ionicons name="ios-star" size={24} color={colors.baseline} /></Text>
                                                    {item.comment?<Text style={styles.ratingText}>"{item.comment}"</Text>:null}
                                                </View>
                                                )
                                            })
                                        }
                    ...
                    <View style={styles.profileInfo}>
                        ...
                        {this.state.currentUser.uname?<Text style={styles.uname}>@{this.state.currentUser.uname}</Text>:null}
                        {this.state.currentUser.prof?<Text style={styles.profession}>{this.state.currentUser.prof}</Text>:null}
                    ...
                    </View>
                    {
                        this.state.sameList && this.state.sameList.length > 0
                            ?
                            <View style={{ width: '90%' }}>
                                <Text style={styles.followedText}>Followed by <ShowSimilar navigation={this.props.navigation} sameList={this.state.sameList} /> {this.state.sameList.length-1>0? ('and '+(this.state.sameList.length-1>1?this.state.sameList.length-1+' others':this.state.sameList.length-1+' other')):null}</Text>
                            </View>
                            :
                            null
                    }
                    ...
                    </View>
                    {
                        auth().currentUser
                            ...
                            <>
                                {
                                    auth().currentUser.email === this.state.currentUser.email
                                        ?
                                        null
                                        :
                                        <View style={styles.actionProfile}>
                                            {
                                                this.state.follow
                                                    ?
                                                    <TouchableOpacity onPress={this.handleFollow} style={styles.followAction}>
                                                        <Ionicons name='ios-person-remove' size={20} color={colors.white} style={{ marginRight: 10 }} />
                                                        <Text style={styles.followActionText}>Unfollow</Text>
                                                    </TouchableOpacity>
                                                    :
                                                    <TouchableOpacity onPress={this.handleFollow} style={styles.followAction}>
                                                        <Ionicons name='ios-person-add' size={20} color={colors.white} style={{ marginRight: 10 }} />
                                            ...
                                            }
                                            {
                                                this.state.initiatingChat
                                                    ...
                                                    <View style={styles.chatAction}>
                                                        <ActivityIndicator color={colors.baseline} size='small' />
                                                    </View>
                                                    :
                                                    <TouchableOpacity onPress={this.handleChat} style={styles.chatAction}>
                                                        <Ionicons name='ios-chatbubble-ellipses' size={26} color={colors.baseline} />
                                                    </TouchableOpacity>
                                            }
                                        </View>
                                }
                    ...
                    <View style={{width:'100%',flex:1,alignItems:'center'}}>
                        ...
                        </View>
                        {
                            this.state.tab==='Posts'
                            ?
                            <OtherPostScreen id={this.state.currentUser._id} navigation={this.props.navigation} location={this.props.route.params.location}/>
                            :
                            null
                        }
                        {
                            this.state.tab==='Ratings'
                            ?
                            <OtherRatingScreen id={this.state.currentUser._id} navigation={this.props.navigation}/>
                            :
                            null
                        }
                    </View>
